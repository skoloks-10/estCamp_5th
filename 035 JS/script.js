...
async function fetchUsers() {
  try {
    const response = await fetch(API_URL); // '/users' 제거
    ...
    renderUsers(users);
    totalUsersCount.textContent = users.length; // 사용자 수 업데이트
  } catch (error) {
    console.error("사용자 조회 중 오류 발생:", error);
    messageContainer.textContent = "사용자 목록을 불러오는데 실패했습니다.";
  }
}
...
async function createUser(userData) {
  // POST /users
  const check = confirm("정말로 생성하시겠습니까?");
  if (!check) {
    return;
  }
  try {
...
async function updateUser(userId, userData) {
  // PUT /users/{user_id}
  const check = confirm("정말로 수정하시겠습니까?");
  if (!check) {
    return;
  }
  try {
